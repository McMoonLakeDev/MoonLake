...
data class PacketOutPayload(
...
) : PacketOutBukkitAbstract("PacketPlayOutCustomPayload") {
    ...
    init {
        if(data.writerIndex() > 1048576)
            throw IllegalArgumentException("自定义数据不能大于 1048576 字节长度.")
    }
    ...
    override fun read(data: PacketBuffer) {
        ...
        if(bytes < 0 || bytes > 1048576)
            throw IllegalArgumentException("自定义数据不能小于 0 或大于 1048576 字节长度.")
