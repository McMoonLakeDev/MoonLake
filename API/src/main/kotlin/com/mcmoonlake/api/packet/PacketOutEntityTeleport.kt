...
data class PacketOutEntityTeleport(
    ...
    override fun write(data: PacketBuffer) {
        ...
        if(!isCombatOrLaterVer) data.writeInt(Math.floor(x * 32.0).toInt())
        else data.writeDouble(x)
        if(!isCombatOrLaterVer) data.writeInt(Math.floor(y * 32.0).toInt())
        else data.writeDouble(y)
        if(!isCombatOrLaterVer) data.writeInt(Math.floor(z * 32.0).toInt())
        else data.writeDouble(z)
